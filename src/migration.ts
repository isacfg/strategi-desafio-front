...
imoveisList.forEach((imovel) => {
  setDoc(doc(collection(db, 'imoveis'), imovel.id.toString()), imovel)
    .then(() => {
      console.log(`Document with ID ${imovel.id} successfully written to Firestore`)
    })
    .catch((error) => {
      console.error(`Error writing document with ID ${imovel.id} to Firestore:`, error)
    })
})
